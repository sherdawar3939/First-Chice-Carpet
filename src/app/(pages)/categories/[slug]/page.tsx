...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/product?subcategory=${slug}`);
    ...
    <div>
      <div className="w-screen justify-center text-center items-center px-5 md:px-20 py-5">
        {products.length === 0 && !loading ? (
          <p className="text-lg">No products found under this category</p>
        ) : (
          <p className="font-bold text-lg text-pretty">{slug}</p>
        )}
      </div>
